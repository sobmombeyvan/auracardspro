<template>
  ...
  <div id="jokard-template-5" style="background-image: var(--jokard-background-image)"
       class="jokard-template relative w-full min-h-screen pb-32 px-2">
    <div class="max-w-lg w-full mx-auto space-y-4">
      <template v-for="(item, index) in blocks!" :key="index">
        <TemplateTemplate5Profile v-if="item.type === 'profile' && item.values.activate" :vcfUrl="vcfUrl"
                                  :preview="preview" :values="(item.values as ProfileBlockProps)"/>
        <TemplateTemplate5HeadingText v-if="item.type === 'heading-text' && item.values.activate" :preview="preview"
                                      :values="(item.values as HeadingTextBlockProps)"/>
        <TemplateTemplate5Contact v-if="item.type === 'contact' && item.values.activate" :preview="preview"
                                  :values="(item.values as ContactBlockProps)"/>
        <TemplateTemplate5SocialLink v-if="item.type === 'social-link' && item.values.activate" :preview="preview"
                                     :values="(item.values as SocialLinkBlockProps)"/>
        <TemplateTemplate5ImageText v-if="item.type === 'image-text' && item.values.activate" :preview="preview"
                                    :values="(item.values as ImageTextBlockProps)"/>
        <TemplateTemplate5Images v-if="item.type === 'images' && item.values.activate" :preview="preview"
                                    :values="(item.values as ImagesBlockProps)"/>
        <TemplateTemplate5Video v-if="item.type === 'video' && item.values.activate" :preview="preview"
                                 :values="(item.values as VideoBlockProps)"/>
        <TemplateTemplate5Pdf v-if="item.type === 'pdf' && item.values.activate" :preview="preview"
