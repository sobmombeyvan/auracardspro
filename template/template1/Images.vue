<template>
  <div :id="values.id" class="jokard-template-image-text flex flex-col gap-4 bg-white overflow-hidden" :class="values.type === 'Slide' ? 'py-4' : 'pt-4'">
    ...
    <div v-else class="grid gap-3" :class=" values.type === 'Grid-1' || values.type === 'Grid-2' ? 'grid-cols-2' : '' ">
      <div v-for="(image, index) in values.images" class="h-[250px]" :class=" index === 0 && values.type === 'Grid-2' ? 'col-span-2'  : ''" :key="index">
        <NuxtImg
            height="500"
            width="800"
            :src="generateUrlFile(image!.path)"
